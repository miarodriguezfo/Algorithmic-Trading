...
def make_pipeline():
    ...
    variable_5 = morningstar.income_statement.normalized_income.latest
    variable_6 =twitter_sentiment.bullish_intensity.latest
    
    variable_1 = morningstar.valuation.market_cap.latest  
    variable_2 = morningstar.Fundamentals.free_cash_flow.latest
    variable_3 = morningstar.valuation.enterprise_value.latest
    ...
    variable_4_w = value.winsorize(min_percentile=0.1, max_percentile=0.9)
    variable_5_w = quality.winsorize(min_percentile=0.1, max_percentile=0.9)
    variable_6_w = sentiment_score.winsorize(min_percentile=0.1,                                                                             max_percentile=0.9)
    ...
    combined_factor = (
        variable_4_w.zscore()*0.4  
        +variable_5_w.zscore()*0.1  
        +variable_6_w.zscore()*0.1
        +variable_1_w.zscore()*0.3
        +variable_2_w.zscore()*0.2
        +variable_3_w.zscore()*0.2
    )
